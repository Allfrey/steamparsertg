...
def parse_profile(message):
    ...
    game1 = game1_element.text.strip().lstrip() if game1_element else 'Нет данных'
    ...
    ban = ban_element.text.strip().split('|')[0].split('Подробнее')[0].strip() if ban_element else 'Нет данных'
    ban_info_element = soup.select_one('.profile_ban_status')
    ban_info = ban_info_element.text.strip().split('|')[1].split('Подробнее')[1].strip() if ban_info_element else 'Нет данных'
